...
public class CustomAuthHandler : SignInAuthenticationHandler<CustomAuthHandlerOptions>
{
    ...
    protected override Task<AuthenticateResult> HandleAuthenticateAsync()
    {
        ...
        else
        {
            ...
            var ticketHelper = new AuthTicketHelper(); 
            var ticket = ticketHelper.DeserializeFromBase64(authCookie);

            var username = ticket?.Principal?.Identity?.Name ?? "Unknown";
    ...
    var ticketHelper = new AuthTicketHelper();
    var serializedTicket = ticketHelper.SerializeToBase64(ticket);


        // Set authentication cookie with configured name and expiration
        Response.Cookies.Append(key: Options.CookieName,value: serializedTicket, new CookieOptions
        {
            HttpOnly = true,
            SameSite = SameSiteMode.Lax
        });

        var redirectUri = properties.RedirectUri  ?? Options.DefaultRedirectPath ?? "/";
